  ...
  if (sensorInfo.device.address) {
    switch (this.model) {
      ...
      case 'BH1750FVI':
        ...
        this.wire = new BH1750FVI(sensOptions);

        this.wire.init(function (err, val) {
          if (err) {
              logger.error('[DigitalLight] BH1750FVI - error with power on', new Error('powermode not set on write'));
          } else {
              logger.debug('[DigitalLight] BH1750FVI - power on');
          }
        });
    ...
    switch (this.model) {
      ...
      case 'BH1750FVI':
        this.wire.getLux(function (err, data) {
          ...
          logger.debug('[DigitalLight] BH1750FVI - data, rtn, info, properties.address - ',
              data, rtn, self.info, DigitalLight.properties.address);
