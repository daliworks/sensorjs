...
var setSensorDriver = function(model, driver) {
  if (!model || !driver) {
    return;
  }
  if (!_cachedDrivers[model]) {
    _cachedDrivers[model] = _cachedDrivers[driver.driverName];
  }
  _.merge(_cachedDrivers[model].properties, driver);
};
...
var getNetwork = function(networkName) {
  ...
  try {
    ...
    logger.error('error to get network [%s]', networkName, e);
  }
...
exports.discover = function (driverName/*or model*/, options, cb) {
  ...
  if (props.discoverable) {
    ...
    if (network) {
      if (options) {
        network.discover(driverName, options, cb);
      } else {
...
exports.discoverThingsOnNetwork = function (networkName, options, cb) {
  var network;
  ...
  if (network) {
    if (options) {
      network.discover(networkName, options, cb);
    } else {
    network.discover(networkName, cb);
    }
  } else {
    return cb && cb(new Error('module not found: ' + networkName));
  }
...
exports.setSensorDriver = setSensorDriver;
