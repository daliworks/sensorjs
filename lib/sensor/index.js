...
var getSensorDriver = exports.getSensorDriver = function (model) {
  ...
  try {
    ...
    try {
      ...
      logger.warn('getSensorDriver() driver not found / model=', model, 'error=', e);
      throw e; 
    }
  }
};
...
exports.getUrl = function(sensorInfo) {
  return ['sensorjs://', sensorInfo.device.protocol, sensorInfo.device.address, sensorInfo.model, sensorInfo.id].join('/');
};
...
var getSensorProperties = exports.getSensorProperties = function (model) {
  try {
    ...
    if (Sensor && Sensor.properties) {
      var props = _.cloneDeep(Sensor.properties); // TODO: cloning on demand
