...
function DigitalCO2(sensorInfo, options) {
  ...
  if (sensorInfo.device.protocol && sensorInfo.device.address) {

    if (sensorInfo.device.protocol === 'i2c') {
      device = I2C_DEVICE + (sensorInfo.device.bus || I2C_BUS_ID);
    } else if (sensorInfo.device.protocol === 'uart') {
    ...
    switch (this.model) {
      case 'X100':
        sensOptions = {
          ...
          device: device,
          protocol: sensorInfo.device.protocol
        };
        ...
        this.wire.init(function (err, val) {
          if (err) {
              logger.error('[DigitalCO2/Driver] X100 - error with power on', new Error('powermode not set on write'));
          } else {
              logger.debug('[DigitalCO2/Driver] X100 - power on');
          }
        });
    ...
    logger.debug('[DigitalCO2/Driver] sensor is created at driver', sensorInfo);
  } else {
    logger.warn('[DigitalCO2/Driver] sensor network protocol or address is not provided');
  }
}
...
DigitalCO2.prototype._get = function () {
  ...
  try {
    switch (this.model) {
      case 'X100':
        switch (this.wire.options.protocol) {
          case 'i2c':
            this.wire.getDensity(function (err, data) {
              ...
              logger.debug('[DigitalCO2/Driver] X100 - data, rtn, info, properties.address - ',
            ...
            break;
          case 'uart':
            logger.debug('[DigitalCO2/Driver] Case X100 - Case uart', this.wire);

            if (this.rtn) {
              logger.debug('[DigitalCO2/Driver] Case X100 - Case uart - this.rtn',
                  this.rtn, this.info, DigitalCO2.properties.address);

              this.emit('data', this.rtn);
            } else {
              this.emit('data', {status: 'error', id : this.id, message: 'CO2 data is not available'});
            }

            break;
          default:
            logger.warn('[DigitalCO2/Driver] ' + this.wire.options.protocol + ' protocol(network) is not supported');
        }
      ...
      default:
        this.wire.readBytes(readLightCmd, 2, function (err, data) {
          ...
          logger.debug('[DigitalCO2/Driver] data, rtn, info, properties.address - ',
              data, rtn, self.info, DigitalCO2.properties.address);
...
DigitalCO2.prototype._enableChange = function () {
  this.onChange = true;  
};
